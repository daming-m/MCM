...
| Claude Code 已安装 | `claude --version` | 版本要求：最新稳定版 |
...
# 验证模板文件已就绪
ls -la
# 应该看到：.claude/  harness-collab/  config/  pom.xml  README.md  AGENTS.md
```
...
```bash
# 验证关键目录和文件存在
Test-Path ".claude/CLAUDE.md"     # 应该看到 True
Test-Path ".claude/settings.local.json"  # 应该看到 True
ls .claude/hooks/     # 应该看到 1 个脚本文件
...
### 步骤 4：在 Claude Code 中打开工作区，验证 Steering 文件已加载

1. 打开终端，进入工作区根目录（`my-workspace/`）
2. 启动 Claude Code：`claude`
3. Claude Code 自动加载 `.claude/CLAUDE.md`
4. 验证约束已生效：
...
### 步骤 5：执行 harness-verify.ps1 验证模板完整性
...
✅ .claude/hooks/harness-reminder.ps1
...
✅ Harness 模板完整性验证通过
```
...
### Q3：Claude Code 没有自动加载 Steering 约束

**原因**：可能是工作区根目录选择不正确，或 `.claude/CLAUDE.md` 不存在。
...
# 如果输出为 0，说明文件为空，需要重新生成

# 重新启动 Claude Code（退出后重新在工作区根目录启动）
```
